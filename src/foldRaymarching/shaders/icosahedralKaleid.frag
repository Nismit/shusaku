...
// Golden-ratio mirror normal of the icosahedral fundamental domain.
// (1, phi, 1/phi) normalized -> unit length, so reflecting against it is
// a pure isometry and the distance estimate stays exact (no breakdown).
const vec3 nIco = vec3(0.5, 0.809016994374947, 0.309016994374947);
...
// abs + single golden-plane reflection, applied three times, folds all of
// space into one icosahedral fundamental triangle. Every step is a
// reflection (isometric), so the creases stay sharp and artifact-free.
vec3 icosaFold(vec3 p) {
  for (int i = 0; i < 3; i++) {
    p = sabs(p);
    p -= 2.0 * min(0.0, dot(p, nIco)) * nIco;
  }
  return p;
}
...
float mapFold(vec3 p, out float orbit) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    orbit += exp(-2.5 * length(q));
  }

  vec3 a = vec3(0.16, 0.42, 0.16);
  return sdBox(q, a) - 0.05;
}
...
void main() {
  ...
  if (hit.x < FAR) {
    ...
    float foldTone = hit.y * 0.08 + 0.42 + length(p) * 0.045;
