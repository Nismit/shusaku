...
// Full octahedral symmetry group fold (order 48), built only from mirror
// reflections: abs (3 axis planes) + descending sort (3 diagonal planes) +
// the body-diagonal plane. Every operation is isometric, so the distance
// estimate is exact and the creases stay sharp without overstep artifacts.
float mapFold(vec3 p, out float orbit) {
  vec3 q = p;
  q.xy = rot(iInitRotXY) * q.xy;
  q.xz = rot(iInitRotXZ) * q.xz;

  float off = iFoldScale * 0.22;
  ...
  for (int i = 0; i < 8; i++) {
    ...
    orbit += exp(-3.0 * length(q));
  }

  return sdOctahedron(q, 0.42) - 0.03;
}
...
void main() {
  ...
  if (hit.x < FAR) {
    ...
    float foldTone = hit.y * 0.08 + 0.50 + length(p) * 0.045;
