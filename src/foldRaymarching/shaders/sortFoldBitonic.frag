...
// order swaps (q.a<->q.b) mirror across a diagonal plane, the sign-aware
// merge swaps (q.a<->-q.b) mirror across an anti-diagonal plane. All are
// isometric, so the distance estimate stays exact and the dense crease
// lattice renders without overstep artifacts. Flipping the sort direction
// each stage builds the characteristic bitonic up/down sequences.
...
void main() {
  ...
  if (hit.x < FAR) {
    ...
    float foldTone = hit.y * 0.08 + 0.65 + length(p) * 0.045;
